...
class ScreeningFormView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk=None):
        ...
        if form.is_valid():
            ...
            # Smart redirect: check previous page (HTTP_REFERER)
            referer = request.META.get('HTTP_REFERER', '').lower()

            if 'form-status-list' in referer:
                return redirect('nanopore:form-status-list')

            elif 'screening-quality' in referer:
                return redirect('reports:screening_quality_report')
    ...
    def get_context_data(self, **kwargs):
        ...
        context["zone_group_1"] = [1]
        context["zone_group_2_5"] = [2, 3, 4, 5]
