...
def global_report_total(request):
    ...
    respecting user roles like in DataQualityReportView.
    """
    report_total = 0
    ...
    # --- Calculate counts for regular sections ---
    ...
    # --- Base total for normal users ---
    report_total = eligible_not_enrolled_count + missing_clinic_count + missing_diagnosis_count \
                   + missing_regimen_count + pending_outcomes_count + pending_outcomes_date_count

    # --- Add Substudy2 only for allowed roles ---
    if is_zonal_lab or is_admin or is_reviewer or request.user.is_superuser:
        report_total += screenings.filter(
