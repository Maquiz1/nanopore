...
class Command(BaseCommand):
    help = "Create or update daily Screening Data Quality snapshot"
    ...
    def handle(self, *args, **options):
        ...
        for g in grouped:
            ...
            ScreeningDQSnapshot.objects.update_or_create(
                snapshot=snapshot,
                zone_id=g["site__district__region__zone_id"],
                site_id=g["site_id"],
                defaults={
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Screening DQ snapshot created/updated for {today}"
            )
        )
