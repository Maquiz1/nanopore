...
class Command(BaseCommand):
    help = "Create daily Diagnosis Data Quality snapshot (per site, per zone)"
    ...
    def handle(self, *args, **options):
        ...
        grouped = qs.values(
            ...
            missing_diagnosis_made_other=Count(
                ...
                filter=Q(
                    tb_diagnosis=1,
                    tb_diagnosis_made__value=96,
                    diagnosis_made_other__isnull=True,
                )
            ),

            missing_tb_diagnosed_clinically=Count(
                "id",
                filter=Q(tb_diagnosis=1, tb_diagnosis_made=1, tb_diagnosed_clinically__isnull=True)
            ),
        ...
        for g in grouped:
            missing_fields = {k: g[k] for k in g if k.startswith("missing_")}
            total_issues = sum(missing_fields.values())

            DiagnosisDQSnapshot.objects.update_or_create(
                snapshot=snapshot,
                zone_id=g["screening__site__district__region__zone_id"],
                site_id=g["screening__site_id"],
                defaults={
        ...
        self.stdout.write(
            self.style.SUCCESS("Diagnosis Data Quality snapshot created/updated successfully ✅")
        )
