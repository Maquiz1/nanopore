...
class Command(BaseCommand):
    help = "Create or update daily Enrollment Data Quality snapshot (per site, per zone)"
    ...
    def handle(self, *args, **options):
        ...
        grouped = qs.values(
            ...
            missing_diseases_medical=Count(
                "id",
                filter=Q(other_diseases=1, diseases_medical__isnull=True)
            ),
            ...
            invalid_regimen_months_with_unknown=Count(
                "id",
                filter=Q(tx_previous=1, regimen_months_unknown=True) &
                       ~Q(regimen_months__isnull=True)
            ),
        )
        ...
        for g in grouped:
            ...
            EnrollmentDQSnapshot.objects.update_or_create(
                snapshot=snapshot,
                zone_id=g["screening__site__district__region__zone_id"],
                site_id=g["screening__site_id"],
                defaults={
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Enrollment DQ snapshot created/updated for {today}")
        )
