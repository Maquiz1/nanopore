# reports/context_processors/diagnosis_context.py
...
def diagnosis_report_total(request):
    ...
    Uses diagnosis_dq service to avoid duplicated logic.
    """

    if not request.user.is_authenticated:
        return {
            "context_diagnosis_report_total": 0
        }

    # No zone/site filters for navbar (global view based on role)
    qs = get_diagnosis_queryset(request.user)
