...
def specific_queries_total(request):
    """
    Aggregates all specific form DQ counts across Screening, Enrollment, Regimen, Diagnosis, Clinic, Zonal.
    Role-aware: only shows what the user should see.
    Returns: {'context_specific_queries_total': total}
    """
    ...
    # Determine role for total calculation
    if is_privileged:
        role = "privileged"
    elif is_zonal_lab:
        role = "zonal_lab"
    ...
    # Get all totals from the unified service
    totals = get_specific_form_dq(request.user, role=role)

    return {"context_specific_queries_total": totals["total_issues"]}
