# reports/services/clinic_dq_counts.py
from django.apps import apps
from django.db.models import Q, Count
from utils.permissions import filter_queryset_by_user_role

def get_clinic_dq_counts(user, zone_id=None, site_id=None):
    ...
    counts = {
        ...
        "missing_number_received": qs.filter(number_required_q, number_received__isnull=True).count(),
        "missing_sample_reason_when_received_2": qs.filter(sample_received_is_2_q, sample_reason__isnull=True).count(),
        "missing_new_reason_when_new_sample_2": qs.filter(new_sample_is_2_q, new_reason__isnull=True).count(),
        "missing_other_reason_when_sample_reason_96": qs.filter(sample_reason_is_96_q, other_reason__isnull=True).count(),

        "missing_date_sample1_collected": qs.filter(number_received__isnull=False, date_sample1_collected__isnull=True).count(),
        ...
        "missing_technique_a": qs.filter(afb_yes_q, technique_a__isnull=True).count(),
        "missing_afb_a_results": qs.filter(afb_yes_q, afb_a_results__isnull=True).count(),
        "missing_afb_b_date": qs.filter(afb_yes_q & Q(afb_a_date__isnull=False, technique_a__isnull=False, afb_a_results__isnull=False) & (Q(afb_b_date__isnull=True) | Q(technique_b__isnull=True) | Q(afb_b_results__isnull=True)), afb_b_date__isnull=True).count(),
        "missing_technique_b": qs.filter(afb_yes_q & Q(afb_a_date__isnull=False, technique_a__isnull=False, afb_a_results__isnull=False) & (Q(afb_b_date__isnull=True) | Q(technique_b__isnull=True) | Q(afb_b_results__isnull=True)), technique_b__isnull=True).count(),
        "missing_afb_b_results": qs.filter(afb_yes_q & Q(afb_a_date__isnull=False, technique_a__isnull=False, afb_a_results__isnull=False) & (Q(afb_b_date__isnull=True) | Q(technique_b__isnull=True) | Q(afb_b_results__isnull=True)), afb_b_results__isnull=True).count(),

        "missing_xpert_mtb_rif_conducted": qs.filter(afb_xpert_required_q, xpert_mtb_rif_conducted__isnull=True).count(),
        "missing_xpert_date": qs.filter(xpert_yes_q, xpert_date__isnull=True).count(),
        "missing_xpert_mtb": qs.filter(xpert_yes_q, xpert_mtb__isnull=True).count(),
        "missing_error_code": qs.filter(xpert_is_8_q, error_code__isnull=True).count(),
        "missing_xpert_rif": qs.filter(xpert_in_2_6_q, xpert_rif__isnull=True).count(),
        "missing_ct_value": qs.filter(xpert_in_2_6_q).exclude(Q(ct_value__isnull=False, ct_na=False) | Q(ct_value__isnull=True, ct_na=True) | Q(ct_value__in=[99,99.0])).count(),
    }
