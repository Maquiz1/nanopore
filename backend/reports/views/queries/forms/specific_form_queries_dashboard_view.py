...
# Import your report count functions (adjust paths as needed)
...
class SpecificFormQueriesDashboardView(TemplateView):
    ...
    #     # screening_total  = get_screening_dq_counts(request.user, zone_id_int, site_id_int)["total"]
    #     # enrollment_total = get_enrollment_dq_counts(request.user, zone_id_int, site_id_int)["total"]
    #     # regimen_total    = get_regimen_dq_counts(request.user, zone_id_int, site_id_int)["total"]
    #     # diagnosis_total  = get_diagnosis_dq_counts(request.user, zone_id_int, site_id_int)["total"]
    #     # clinic_total     = get_clinic_dq_counts(request.user, zone_id_int, site_id_int)["total"]
    #     # zonal_total      = get_zonal_dq_counts(request.user, zone_id_int, site_id_int)["total"]

    #     screening_total  = get_screening_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)
    #     enrollment_total = get_enrollment_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)
    #     regimen_total    = get_regimen_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)
    #     diagnosis_total  = get_diagnosis_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)
    #     clinic_total     = get_clinic_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)
    #     zonal_total      = get_zonal_dq_counts(request.user, zone_id_int, site_id_int).get("total", 0)

    #     # ── Compute combined total based on role ──
    #     if is_privileged:
    #         specific_queries_total = (
    #             screening_total + enrollment_total + regimen_total +
    #             diagnosis_total + clinic_total + zonal_total
    #         )
    #     elif is_zonal_lab:
