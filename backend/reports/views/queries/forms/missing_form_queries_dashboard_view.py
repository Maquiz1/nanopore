...
class MissingFormQueriesDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        selected_zone_name = zones.get(zone_id_int, "All Zones")
        selected_site_name = sites.get(site_id_int, "All Sites")

        # ── Missing form counts (SERVICE) ────────────────────────────
        missing_counts = get_missing_forms_counts(
            request.user,
            zone_id=zone_id_int,
            site_id=site_id_int,
        )
