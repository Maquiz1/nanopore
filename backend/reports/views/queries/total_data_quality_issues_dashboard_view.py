...
from reports.services.total_dq import get_total_data_quality_overview


class TotalDataQualityDashboardView(TemplateView):
    template_name = (
        "reports/data_quality/query_dashboard/"
        "all_queries_overview_dashboard.html"
    )
    ...
    def get_context_data(self, **kwargs):
        ...
        # ── Determine role string (SAME STANDARD) ──
        ...
        # ── Fetch overview (NOW ROLE-AWARE) ──
        overview = get_total_data_quality_overview(
            user=request.user,
            zone_id=zone_id_int,
            site_id=site_id_int,
            role=role,   # ✅ aligned with other dashboards
        )
